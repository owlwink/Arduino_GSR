...
void setup() {
  ...
  threshold = 3.32 - (sum / 500 * (5.0 / 1023.0)); //average conductance threshold calculated by subtracting max value by sum divided by 500 multiplied by the converted analog to voltage value
  thresholdMS = threshold * 1000000; //average threshold in micro-Siemens calculated by multipling threshold above by micro-Siemen conversion rate
  //Serial.print("threshold = ");
  //Serial.println(threshold);
  //Serial.print('\n'); //carriage return for easier display
  //Serial.print("thresholdMS = ");
  //Serial.println(thresholdMS);
  //thresholdP = (thresholdMS / 100) * 10; //establising 10% of the threshold value
  //Serial.println(thresholdP);
}
...
if (conductanceMS <= (thresholdMS / 100) * 90){ //execute serial print when 10% less than threshold
  Serial.print(0);
}
if (conductanceMS > (thresholdMS / 100) * 110){ //execute serial print when 10% more than threshold
  Serial.print(1);
}
